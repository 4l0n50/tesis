...
Sintactically, a NIZK proof system consists of three probabilistic polynomial time algorithms: a CRS
...
In this work we will consider two particular cases of NIZK: \emph{Composable} NIZK \cite{EC:GroSah08} and  \emph{Quasi-Adaptive} NIZK \cite{AC:JutRoy13}. Both deals with the case of \emph{CRS dependent languages}, say the language language is parametrized by some values which are (randomly) sampled within the CRS. Concretely, the CRS might contain the group key $gk$ defining a bilinear group and the language might be some set of satisfiable equations over that gruoup. Quasi-Adaptive NIZK goes even further, the language might also depend on group constants defined in the CRS. In this case the Zero-Knowledge definition is more complex since knowledge of the associated discrete logs might trivialize the construction of a simulator.
