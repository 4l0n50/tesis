...
the proof of equal commmitment opening in Sect.~\ref{sec:aggcommit}. Indeed, the concatenation of $n$ GS commitments is just a commitment to a vector of scalars. In particular,  given $\mathsf{crs}_\GS=(\Gamma,[\vecb{u}_1]_1,[\vecb{u}_2]_1,[\vecb{v}_1]_2,[\vecb{v}_2]_2)$,  it is easy to see that  $n$ commitments to $x_i \in \Z_q$, which are of the form:
 ...
 $$
and similarly the concatenation of $n$ commitments $[\vecb{d}_i]_2$, $i \in [\ell]$ can be written as $[\matr{V}_1]_2\vecb{y}+[\matr{V}_2]_2 \vecb{s}$, where $[\matr{V}_i]_2$ is the block-wise concatenation of $n$ copies of $[\vecb{v}_i]_2$.  
...
\ref{sec:gs-proofs}.  Let $\dist_{\Gamma}$ be a witness samplable distribution which outputs $n$ pairs of vectors
...
\begin{description}
\item[$\algK_0(1^\lambda)$:]  Return $\Gamma := (q,\Gr,\Hr,\GG_T,e,\mathcal{P}_1,\mathcal{P}_2) \leftarrow \ggen_a(1^{\lambda})$.
...
\end{theorem}  
\begin{proof}\ \\
{\textbf{Completeness.}}  Observe that for each $i\in[k]$
\begin{equation}
...
$\iota_2(a_{\ell,i} \beta_{\ell,j})$, and the latter is part of the CRS.
 ...
 \noindent{\textbf{Soundness.}} We change to a game $\mathsf{Game}_{1}$ where we know the discrete logarithm of the GS commitment key, as well as the discrete logarithms of $(\grkb{\alpha}_{\ell},\grkb{\beta}_{\ell})$, $\ell \in [n]$. This is possible because they are both chosen from a witness samplable distribution.
...
\noindent{\textbf{Zero-Knowledge.}}  The same simulator of GS proofs can be used. Specifically
...
in this case, in the soundness proof, the adversary $\advB$ receives $[\matr{A}]_2\in\Hr^{n\times k}$, an instance of the $\dist_{n,k}-\kermdh_{\Hr}$ problem. The adversary $\advB$ outputs $[\grkb{\rho}]_1:=(\grkb{\beta}_1^\top[\vecb{x}]_1 -[{t}_{1}]_1,\ldots,
